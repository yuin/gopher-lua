...
func newLabelDesc(id int, name string, pc, line, n int) *labelDesc {
	return &labelDesc{
		Id:                 id,
		Name:               name,
		Pc:                 pc,
...
func (l *labelDesc) SetNumActiveLocalVars(n int) {
...
func (b *codeBlock) OnFindUnresolvedGoto(index int) {
	b.firstGotoIndex = index
}

func (b *codeBlock) AddLabel(label *labelDesc) {
	b.labels[label.Name] = label
}

func (b *codeBlock) GetLabel(label string) *labelDesc {
...
type funcContext struct {
	...
	labelId              int
	labelPc              map[int]int
	unresolvedGotosCount int
	unresolvedGotos      map[int]*labelDesc
}
...
func (fc *funcContext) CheckUnresolvedGoto() {
	for i := fc.Block.firstGotoIndex; i < fc.unresolvedGotosCount; i++ {
...
func (fc *funcContext) AddUnresolvedGoto(label *labelDesc) {
	fc.unresolvedGotos[fc.unresolvedGotosCount] = label
	fc.Block.OnFindUnresolvedGoto(fc.unresolvedGotosCount)
	fc.unresolvedGotosCount++
}

func (fc *funcContext) AddNamedLabel(label *labelDesc) {
	fc.Block.AddLabel(label)
	fc.SetLabelPc(label.Id, fc.Code.LastPC())
}

func (fc *funcContext) GetNamedLabel(name string) *labelDesc {
	return fc.Block.GetLabel(name)
}

func (fc *funcContext) ResolveGoto(from, to *labelDesc, index int) {
	if from.NumActiveLocalVars < to.NumActiveLocalVars {
		varName := fc.LocalVars()[to.NumActiveLocalVars-1].Name
...
func (fc *funcContext) FindLabel(block *codeBlock, gotoLabel *labelDesc, i int) bool {
	target := block.GetLabel(gotoLabel.Name)
	if target != nil {
		fc.ResolveGoto(gotoLabel, target, i)
		if gotoLabel.NumActiveLocalVars > target.NumActiveLocalVars &&
			(block.RefUpvalue || fc.unresolvedGotosCount > block.firstGotoIndex) {
			fc.PatchJump(gotoLabel.Pc, target.NumActiveLocalVars)
		}
		return true
	}
	return false
...
func (fc *funcContext) ResolveCurrentBlockGotosWithParentBlock() {
	...
	parentLocalVars := len(fc.Block.Parent.LocalVars.Names())
	for i := fc.Block.firstGotoIndex; i < fc.unresolvedGotosCount; i++ {
		gotoLabel, ok := fc.unresolvedGotos[i]
		if !ok {
			break
		}
		if gotoLabel.NumActiveLocalVars > parentLocalVars {
			if fc.Block.RefUpvalue {
				fc.PatchJump(gotoLabel.Pc, parentLocalVars)
			}
			gotoLabel.SetNumActiveLocalVars(parentLocalVars)
		}
		fc.FindLabel(fc.Block.Parent, gotoLabel, i)
	}
}

func (fc *funcContext) ResolveForwardGoto(target *labelDesc) {
	for i := fc.Block.firstGotoIndex; i <= fc.unresolvedGotosCount; i++ {
		gotoLabel, ok := fc.unresolvedGotos[i]
		if !ok {
			break
...
func (fc *funcContext) PatchJump(pc, level int) {
	level++
	fc.Code.SetA(pc, level)
}
...
func compileLabelStmt(context *funcContext, stmt *ast.LabelStmt) { // {{{
...
func compileGotoStmt(context *funcContext, stmt *ast.GotoStmt) { // {{{
	context.Code.AddASbx(OP_JMP, 0, 0, sline(stmt))
	...
	context.FindLabel(context.Block, label, context.unresolvedGotosCount-1)
} // }}}
...
func patchCode(context *funcContext) { // {{{
	...
	for pc := 0; pc < len(code); pc++ {
		...
		case OP_JMP: // resolve pseudo-label and jump to jump optimization
			totalDistance := 0
			currentPc := pc
			maxJumps := 20
			count := 0
			a := opGetArgA(inst)
			for jmp := inst; opGetOpCode(jmp) == OP_JMP && count < maxJumps; jmp = context.Code.At(currentPc) {
				context.Code.SetA(currentPc, a)
				d := 0
				if currentPc >= pc {
					d = context.GetLabelPc(opGetArgSbx(jmp)) - currentPc
				} else {
					d = opGetArgSbx(jmp) + 1
				}
				if (totalDistance + d) > opMaxArgSbx {
					break
				}
				totalDistance += d
				currentPc = currentPc + d + 1
				count++
			}
			if count == maxJumps {
				raiseCompileError(context, context.Proto.LineDefined, "possibly infinite loop")
			}
			context.Code.SetSbx(pc, totalDistance)
		default:
...
func Compile(chunk []ast.Stmt, name string) (proto *FunctionProto, err error) { // {{{
	...
	funcexpr.SetLastLine(eline(chunk[len(chunk)-1]))
