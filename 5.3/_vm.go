...
func mainLoop(L *LState, baseframe *callFrame) {
	...
	for {
		...
		cf.Pc++
		if jumpTable[opGetOpCode(inst)](L, inst, baseframe) == 1 {
			return
		}
	}
}
...
func init() {
	jumpTable = [opCodeMax + 1]instFunc{
		...
		opArith,          // OP_MUL
		opArith,          // OP_DIV
		opNotImplemented, // OP_IDIV
		opNotImplemented, // OP_BAND
		opNotImplemented, // OP_BOR
		opNotImplemented, // OP_BXOR
		opNotImplemented, // OP_SHL
		opNotImplemented, // OP_SHR
		opArith,          // OP_MOD
		opArith,          // OP_POW
...
func opNotImplemented(L *LState, inst uint32, baseframe *callFrame) int {
